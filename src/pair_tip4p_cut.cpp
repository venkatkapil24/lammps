...
void PairTIP4PCut::compute(int eflag, int vflag)
{
  ...
  double rsq,r2inv,rinv,rr,rr2,rr3,rr4,forcecoul,factor_coul,fsc,der_fsc;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsqplus) {
        ...
        if (rsq < cut_coulsq) {
          ...
          fsc = 1 - 2.0 * (rr) + 2.0 * rr3 - rr4;
          der_fsc = (-2.0 + 6.0 * rr2 - 4.0 * rr3) / sqrt(cut_coulsq);
          
          forcecoul = qqrd2e * qtmp * q[j] * (rinv * fsc - der_fsc);
        ...
        // vlist stores 2,4,6 atoms whose forces contribute to virial
          ...
          if (eflag) {
            ecoul = qqrd2e * qtmp * q[j] * sqrt(r2inv) * fsc;
            ecoul *= factor_coul;
          } else ecoul = 0.0;
